...
Le site applique la règle de l'espace insécable : **pleine** (` `, U+00A0) avant `:`, **fine** (` `, U+202F) avant `;`, `!`, `?`, et de part et d'autre des guillemets `« »`. Sans elle, un navigateur peut couper la ligne juste avant le signe et le laisser seul en début de ligne suivante — visuellement fautif, et ça arrive plus souvent qu'on ne croit sur une page qui change de largeur.

**`build.py` s'en charge tout seul** pour les deux manuels : la fonction `typographie_fr()` s'applique en toute dernière étape, sur la page HTML complète déjà assemblée — jamais sur le `.md` source, qui reste en espaces normaux, lisibles et simples à retaper. Elle est **consciente des balises** : elle ignore tout ce qui est à l'intérieur d'une balise ou d'un attribut (`href="..."`, `class="a:b"`) et le contenu d'un `<script>` ou d'un `<style>`, pour ne jamais abîmer du code en cherchant à corriger de la prose.

**Les pages statiques** (`index.html`, `creation.html`, `fiche.html`, `licence.html`, `table.html`, `actions.html`) ne repassent par aucun générateur : la correction leur a été appliquée une fois, directement, avec la même fonction. Si tu ajoutes du texte à la main dans l'une d'elles ensuite, retape les espaces normalement — inutile de saisir un caractère insécable à la main, ça n'a pas besoin d'être fait à chaque fois, seulement de loin en loin en relançant la fonction sur le fichier.

**Une exception délibérée.** Un texte affiché dynamiquement par du JavaScript (le petit lanceur de dé de l'accueil) n'existe pas encore au moment où `typographie_fr()` tourne : il est écrit à la main dans le code, avec l'échappement `\u00a0` directement dans la chaine de caractères — plus lisible dans le code source qu'un octet invisible collé au milieu d'un texte.
